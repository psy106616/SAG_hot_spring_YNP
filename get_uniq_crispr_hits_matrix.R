# based on the blast result, create the unique crispr signals matrix
...
# read-in the blast results for crispr spacers
...
# get the query reads for those who only have hit in one viral partition
uniq_recs_row_idx = !(duplicated(data_id_no_na[,2]) | duplicated(data_id_no_na[,2], fromLast = TRUE))

# change the matrix format into list format
uniq_recs_list = data_id_no_na[uniq_recs_row_idx, ]
dup_rec_list = data_id_no_na[!uniq_recs_row_idx, ]

# for the queries has multiple matches in different viral partition, it will be assigned to the one with most matches already based on the previous results on the unique viral partition
...
# this part is to include all multiple matches in different viral partitions
